...
namespace Itanio.Autenticacao.Repositorios
{
    public class GrupoAcessoRepository
    {
        ...
        {
            var resultados = _gerenciadorConexao.Conexao.QueryMultiple(SQL_ObterPorId + SQL_ListarUsuariosPorGrupo + SQL_ListarPermissoesPorGrupo, new { id });
        ...
        private const string SQL_Alterar = @"UPDATE GrupoAcesso Nome = @Nome, Ativo = @Ativo, DataHoraUltimaAlteracao = GETDATE() WHERE Id = @Id;";
        ...
        private const string SQL_ListarUsuariosPorGrupo = @"SELECT u.* FROM Usuario u inner join GrupoAcessoUsuario gu on gu.idusuario = u.id where IdGrupoAcesso = @id";
        private const string SQL_ListarPermissoesPorGrupo = @"SELECT p.* FROM Permissao p inner join GrupoAcessoPermissao gp on gp.idPermissao = p.id where Id = @id";
        
        private const string SQL_AdicionarPermissao = @"INSERT INTO GrupoAcessoPermissao (idPermissao, idGrupoAcesso) values(@idPermissao, @idGrupoAcesso)";
        private const string SQL_RemoverPermissao = @"DELETE FROM GrupoAcessoPermissao where idPermissao = @idPermissao and idGrupoAcesso = @idGrupoAcesso";
        private const string SQL_AdicionarUsuario = @"INSERT INTO GrupoAcessoUsuario (idUsuario, idGrupoAcesso) values(@idUsuario, @idGrupoAcesso)";
        private const string SQL_RemoverUsuario = @"DELETE FROM GrupoAcessoUsuario where idUsuario = @idUsuario and idGrupoAcesso = @idGrupoAcesso";
    }
}
